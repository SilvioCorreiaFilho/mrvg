    ...
    <header className={`fixed w-full z-50 transition-all duration-500 ${animationClass} ${isScrolled ? 'bg-gray-900/70 backdrop-blur-xl shadow-lg py-4' : 'bg-transparent py-8'}`}>
      <div className="container mx-auto px-6 flex justify-between items-center">
        ...
        <nav className="hidden md:flex items-center space-x-12 uppercase text-xs tracking-widest">
          <a href="#sobre" className={`transition-all duration-300 ease-in-out font-medium relative group flex items-center gap-2 ${isScrolled ? 'text-gray-800 hover:text-gray-900' : 'text-white hover:text-white'}`}>
          ...
          <a href="#atuacao" className={`transition-all duration-300 ease-in-out font-medium relative group flex items-center gap-2 ${isScrolled ? 'text-gray-800 hover:text-gray-900' : 'text-white hover:text-white'}`}>
            <FaGavel className="text-sm" />
            Áreas de Atuação
            <span className={`absolute -bottom-1 left-0 w-0 h-[1px] group-hover:w-full transition-all duration-300 ${isScrolled ? 'bg-gray-800' : 'bg-white'}`}></span>
          </a>
          <a href="#equipe" className={`transition-all duration-300 ease-in-out font-medium relative group flex items-center gap-2 ${isScrolled ? 'text-gray-800 hover:text-gray-900' : 'text-white hover:text-white'}`}>
            <FaUsers className="text-sm" />
            Equipe
            <span className={`absolute -bottom-1 left-0 w-0 h-[1px] group-hover:w-full transition-all duration-300 ${isScrolled ? 'bg-gray-800' : 'bg-white'}`}></span>
          </a>
          <a href="#blog" className={`transition-all duration-300 ease-in-out font-medium relative group flex items-center gap-2 ${isScrolled ? 'text-gray-800 hover:text-gray-900' : 'text-white hover:text-white'}`}>
            <FaNewspaper className="text-sm" />
            Blog
            <span className={`absolute -bottom-1 left-0 w-0 h-[1px] group-hover:w-full transition-all duration-300 ${isScrolled ? 'bg-gray-800' : 'bg-white'}`}></span>
          </a>
          <a href="#contato" className={`border border-opacity-70 px-7 py-2 transition-all duration-300 ease-in-out font-medium flex items-center gap-2 ${isScrolled ? 'border-gray-800 text-gray-800 hover:bg-gray-800 hover:bg-opacity-10' : 'border-white text-white hover:bg-white hover:bg-opacity-10'}`}>
